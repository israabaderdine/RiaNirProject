    ...
    if nir_files and lab_file:
        ...
        for file in nir_files:
            ...
            if not temp_spectrum.empty:
                base_batch = re.sub(r'_scan\d+$', '', temp_batch)
        ...
        # 🔥 CRITICAL: Extract base batch for grouping (remove _scan suffix)
        groups = clean_data['Batch Number'].apply(lambda x: x.split('_')[0]).values
        
        # Show batch distribution
        st.info(f"📊 Found {len(np.unique(groups))} unique batches with {len(groups)} total spectra")

        # =============================
        # 2️⃣ PROPER BATCH-BASED SPLIT (NO LEAKAGE!)
        # =============================
        from sklearn.model_selection import GroupShuffleSplit
        
        test_size = st.slider("Test set size (% of batches)", min_value=10, max_value=40, value=20) / 100
        ...
        # Show split info
        train_batches = np.unique(groups[train_idx])
        test_batches = np.unique(groups[test_idx])
        
        col1, col2 = st.columns(2)
        with col1:
            st.success(f"✅ Training: {len(train_batches)} batches ({len(X_train)} spectra)")
            st.write("Training batches:", ", ".join(train_batches[:5]) + ("..." if len(train_batches) > 5 else ""))
        with col2:
            st.success(f"✅ Testing: {len(test_batches)} batches ({len(X_test)} spectra)")
            st.write("Testing batches:", ", ".join(test_batches[:5]) + ("..." if len(test_batches) > 5 else ""))

        # =============================
        # 3️⃣ AUGMENTATION (TRAIN ONLY)
        # =============================
        use_augmentation = st.checkbox("Enable Data Augmentation", value=False, 
                                      help="Generate synthetic spectra to improve robustness")

        if use_augmentation:
            with st.expander("⚙️ Augmentation Settings", expanded=True):
                aug_factor = st.slider("Augmentation factor", min_value=1, max_value=10, value=3)
                noise_level = st.slider("Noise level", min_value=0.0001, max_value=0.01, value=0.001, format="%.4f")
                
                wavelengths_float = [float(w) for w in wavelength_cols]
                
                X_aug, y_aug, _ = create_augmented_dataset(
                    X_train,
                    y_train,
                    wavelengths_float,
                    groups[train_idx],
                ...
                st.info(f"📈 Training samples after augmentation: {len(X_train)} → {len(X_train_final)}")
        ...
        model_choice = st.radio(
            ...
            [
                  "📊 PLS Regression",
                "🌲 Random Forest (Best for small datasets)", 
             "⚡ XGBoost", 
           ],
            index=0
        )
        ...
        if "Random Forest" in model_choice:
            st.info("🌲 Random Forest is robust and works well with your 28 batches")
            ...
            X_train_final_model = X_train_final
            X_test_eval = X_test
            ...
            from sklearn.cross_decomposition import PLSRegression
            n_components = st.slider("PLS Components", 2, 15, 8)
            
            # PLS needs SNV normalization
            X_train_final_model = apply_snv(X_train_final)
            X_test_eval = apply_snv(X_test)
            
            model = PLSRegression(n_components=n_components)

        # =============================
        # 5️⃣ TRAIN MODEL BUTTON
        # =============================
        if st.button("🚀 Train Model", type="primary"):
            with st.spinner("Training in progress..."):
                if "PLS" in model_choice:
                    model.fit(X_train_final_model, y_train_final)
                else:
                    model.fit(X_train_final_model, y_train_final)
            ...
            st.session_state.model_data = {
                ...
                'dataset_info': {
                    'n_total_batches': len(np.unique(groups)),
                    'n_train_batches': len(train_batches),
                    'n_test_batches': len(test_batches),
                    'n_spectra': len(clean_data),
                    'n_augmented': len(X_train_final) - len(X_train) if use_augmentation else 0,
                    'wavelengths': len(wavelength_cols),
            ...
            st.rerun()  # Rerun to show the evaluation and save section

        # =============================
        # 6️⃣ EVALUATION (Show if model exists in session state)
        # =============================
        if st.session_state.get('trained_model') is not None:
            st.markdown("## 📊 Performance on Unseen Batches")
            ...
            for i, target in enumerate(existing_targets):
                ...
                # Status with better thresholds for your data
                if r2 >= 0.7:
                    status = "🏆 Excellent"
                elif r2 >= 0.5:
                    status = "✅ Good"
            ...
            # Show prediction vs actual for first target
            # Show prediction vs actual for all targets
            st.markdown("### 📈 Prediction vs Actual (Test Batches)")
            ...
            fig = make_subplots(
                rows=n_rows, 
                cols=n_cols,
                subplot_titles=existing_targets,
                x_title="Actual Values",
            ...
            for i, target in enumerate(existing_targets):
                ...
                fig.add_trace(
                    go.Scatter(
                        ...
                        y=y_pred[:, i],
                        mode='markers',
                        marker=dict(color='#764ba2', size=8, opacity=0.7),
                        name=target,
                        text=[f"Batch: {b}" for b in groups[test_idx]],
                        hovertemplate='<b>Batch: %{text}</b><br>Actual: %{x:.2f}<br>Predicted: %{y:.2f}<extra></extra>',
                        showlegend=False
                    ),
                    row=row, col=col
                )
            ...
            # fig.update_layout(
            #     title=f"{existing_targets[0]} - Prediction vs Actual",
            ...
            # =============================
            # 7️⃣ SAVE MODEL (Always visible after training)
            # =============================
            st.markdown("## 💾 Save Model")
            st.info("✅ Model is ready to save. Click the button below to save it permanently.")

            model_name_input = st.text_input(
                "Model Name",
                value=f"model__{datetime.now().strftime('%Y%m%d_%H%M')}"
            )
            ...
            if st.button("💾 Save Model", type="primary", use_container_width=True):
                ...
                st.success(f"✅ Model saved successfully as `{model_name_input}.pkl`")
                st.balloons()
        ...
        st.warning("👆 Please upload NIR files and Lab file to begin training")
elif mode == "🔮 Predict":
