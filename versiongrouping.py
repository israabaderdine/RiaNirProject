...
if mode == "🏠 Dashboard":
    ...
    if nir_files and lab_file:
        ...
        # 🔥 VERY IMPORTANT — extract BASE batch for grouping
        groups = clean_data['Batch Number'].apply(lambda x: x.split('_')[0]).values

        # =============================
        # 2️⃣ GROUP SPLIT (NO LEAKAGE)
        # =============================
        from sklearn.model_selection import GroupShuffleSplit
        ...
        st.success(f"Training batches: {len(np.unique(groups[train_idx]))}")
        st.success(f"Testing batches: {len(np.unique(groups[test_idx]))}")
        ...
        if use_augmentation:
            ...
            st.info(f"Training samples after augmentation: {len(X_train_final)}")
        ...
        with st.spinner("Training model..."):
            ...
            if "Random Forest" in model_type:
                ...
                n_components = min(15, X_train_snv.shape[0]-1, X_train_snv.shape[1])
       ...
       # =============================
        # 5️⃣ EVALUATION (ALWAYS SHOW TABLE)
        # =============================
        st.markdown("## 📈 Test Performance (Unseen Batches)")
        ...
        # =============================
        # 6️⃣ SAVE MODEL SECTION (ALWAYS VISIBLE)
        # =============================
        ...
        if st.button("💾 Save Model", type="primary", use_container_width=True):
            ...
            dataset_info = {
                'n_samples_total': len(clean_data),
                'n_train_samples': len(X_train_final),
                'n_test_samples': len(X_test),
                'n_wavelengths': len(wavelength_cols),
                'targets': existing_targets,
                'train_batches': list(np.unique(groups[train_idx])),
                'test_batches': list(np.unique(groups[test_idx])),
        ...
        st.warning("Upload NIR files and Lab file first.")
