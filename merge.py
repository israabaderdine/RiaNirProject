...
def preprocess_spectra(X, method='snv'):
    ...
    return X
def predict_with_multi_model(multi_models, spectrum, wavelengths):
    """Make predictions using multiple models for different targets"""
    predictions = {}
    ...
    try:
        ...
        for model_name, model_info in multi_models.items():
            if model_info['type'] == 'pls':
                # PLS needs SNV normalization
                X_new_processed = apply_snv(X_new)
                
                if model_name == 'pls':
                    # Multi-target PLS model
                    pred = model_info['model'].predict(X_new_processed)
                    for idx, target in enumerate(model_info['targets']):
                        value = pred[0, idx]
                        predictions[target] = max(0, value)  # Clip negative
                        
            elif model_info['type'] == 'xgb':
                # XGBoost doesn't need normalization
                pred = model_info['model'].predict(X_new)
                value = pred[0] if isinstance(pred, np.ndarray) and pred.shape[0] > 0 else pred
                predictions[model_info['target']] = max(0, value)
...
def get_model_selector():
    ...
    with col1:
        model_type = st.radio(
            ...
            key="model_type_radio"  # Add unique key
        )
    ...
    if "Random Forest" in model_type:
        ...
        with col2:
            params['min_samples_split'] = st.slider(
                ...
                key="rf_min_samples_split"  # Add unique key
            )
            params['min_samples_leaf'] = st.slider(
                "أقل عدد في الورقة",
                min_value=1, max_value=5, value=2, step=1,
                help="يمنع overfitting",
                key="rf_min_samples_leaf"  # Add unique key
            )
...
if mode == "🏠 Dashboard":
    ...
    if nir_files and lab_file:
        ...
        # ============================================
        # ============================================
        # MODEL SELECTION SECTION - UPDATED (NO RECURSION)
        # ============================================
        st.markdown("### 🤖 Model Selection")

        # Option for using different models for different targets
        use_multi_model = st.checkbox(
            "🎯 Use different models for different targets", 
            value=False,
            help="Apply PLS on Protein & Moisture, XGBoost on Fat, Ash, Fiber & wa",
            key="use_multi_model_checkbox"  # Add unique key
        )

        if use_multi_model:
            st.markdown("""
            <div style='background: linear-gradient(135deg, #667eea20 0%, #764ba220 100%); padding: 1rem; border-radius: 8px; margin-bottom: 1rem;'>
                <h4 style='color: #764ba2; margin: 0;'>Multi-Model Configuration</h4>
                <p style='margin-top: 0.5rem; font-size: 0.9rem;'>
                    📊 <strong>PLS Regression</strong>: Protein, Moisture<br>
                    ⚡ <strong>XGBoost</strong>: Fat, Ash, Fiber, wa
                </p>
            </div>
            """, unsafe_allow_html=True)
            
            # Configuration for PLS targets
            with st.expander("🔧 PLS Regression Settings (Protein, Moisture)", expanded=False):
                col1, col2 = st.columns(2)
                with col1:
                    pls_components = st.slider(
                        "PLS Components",
                        min_value=2,
                        max_value=min(20, X_snv.shape[0] - 1, X_snv.shape[1]),
                        value=min(10, X_snv.shape[0] - 1, X_snv.shape[1]),
                        help="Number of PLS components for Protein and Moisture",
                        key="pls_components_slider"  # Add unique key
                    )
                with col2:
                    st.info("PLS is good for linear relationships and works well with few samples")
            
            # Configuration for XGBoost targets
            with st.expander("⚡ XGBoost Settings (Fat, Ash, Fiber, wa)", expanded=False):
                col1, col2 = st.columns(2)
                with col1:
                    xgb_estimators = st.slider(
                        ...
                        value=200,
                        step=50,
                        help="More estimators = better but slower",
                        key="xgb_estimators_slider"  # Add unique key
                    )
                    xgb_learning_rate = st.select_slider(
                        ...
                        key="xgb_learning_rate_slider"  # Add unique key
                    )
                with col2:
                    xgb_max_depth = st.slider(
                        ...
                        value=6,
                        step=1,
                        help="Deeper trees = more complex",
                        key="xgb_max_depth_slider"  # Add unique key
                    )
                    xgb_subsample = st.select_slider(
                        "Subsample",
                        options=[0.6, 0.7, 0.8, 0.9, 1.0],
                        value=0.8,
                        help="Fraction of samples to use",
                        key="xgb_subsample_slider"  # Add unique key
                    )
            
            # Set model_type for display purposes
            model_type = "Multi-Model (PLS + XGBoost)"
            model_params = {}  # Empty params for multi-model
            
        else:
            # Original single model selection - call the function directly
        ...
        with col_save1:
            # Update model saving to handle multi-models
            if st.button("💾 Save Model", type="primary", use_container_width=True):
                ...
                # Add multi-model info if used
                if use_multi_model and hasattr(st.session_state, 'multi_models'):
                    dataset_info['multi_model_config'] = {
                        'pls_targets': [t for t in ['Protein', 'moisture'] if t in existing_targets],
                        'xgb_targets': [t for t in ['fat', 'ash', 'Fiber', 'wa'] if t in existing_targets]
                    }
                    # Save multi-models
                    model_data_to_save = {
                        'model': st.session_state.multi_models,
                ...
                st.session_state.model_data = model_data_to_save
        ...
        for file in nir_files:
            with st.expander(f"📄 {file.name}", expanded=True):
                ...
                if not spectrum.empty:
                        # In the prediction section, replace the prediction call with:
                        if st.session_state.model_data:
                            # Check if we have multi-models or single model
                            if hasattr(st.session_state, 'multi_models') and st.session_state.multi_models:
                                result_df, aligned_spectrum = predict_with_multi_model(
                                    st.session_state.multi_models,
                                    spectrum,
                                    st.session_state.model_data['wavelengths']
                                )
                            else:
